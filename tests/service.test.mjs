...
/**
 * 验证同一个超时 turn 的通知在达到 Bark 重试上限后会直接收口。
 * 后续轮询不再重复请求，也不再持续写入同类失败日志。
 */
test('超时 turn 通知失败达到上限后不会再次重试', async () => {
  const sessionsDir = await createTempDir();
  const runtimeDir = await createTempDir();
  let requestCount = 0;
  const server = http.createServer((request, response) => {
  ...
  try {
    ...
    const filePath = await writeSessionFile(sessionsDir, 'stale-failed-once.jsonl', [
      JSON.stringify({
        timestamp: '2026-04-12T16:39:10.000Z',
        type: 'session_meta',
        payload: {
          id: 'session-stale-once',
          originator: 'Codex Desktop',
          source: 'vscode'
        }
      }),
      JSON.stringify({
        timestamp: '2026-04-12T16:39:20.000Z',
        type: 'event_msg',
        payload: {
          type: 'task_started',
          turn_id: 'turn-stale-once'
        }
      })
    ]);
    ...
    service.config.service.staleTurnTimeoutMs = 1;
    const fileState = service.stateStore.getState().files[filePath];
    fileState.turns['turn-stale-once'].lastEventAt = '2026-04-12T16:39:20.000Z';

    await service.checkStaleTurns();
    await service.checkStaleTurns();

    assert.equal(requestCount, 1);
    assert.equal(fileState.turns['turn-stale-once'], undefined);

    const logContent = await fs.readFile(service.logFilePath, 'utf8');
    assert.equal(
      logContent.includes('超时错误通知发送失败，当前 turn 已停止继续重试'),
      true
    );
    assert.equal(
      logContent.match(/超时错误通知发送失败，当前 turn 已停止继续重试/g)?.length ?? 0,
      1
    );
  } finally {
    await new Promise((resolve, reject) => {
...
/**
 * 验证同类项目内部错误在同一轮失败后不会立刻重新发起新的 Bark 请求。
 * 这样可以把失败后的重试严格收束在 `push.maxAttempts` 之内，避免主循环重复放大。
 */
test('项目内部异常通知失败后，同一错误不会立刻再次尝试发送', async () => {
  const sessionsDir = await createTempDir();
  const runtimeDir = await createTempDir();
  let requestCount = 0;
  const requests = [];
  ...
  try {
    ...
    assert.equal(requestCount, 1);
    assert.ok(requests[0].includes('title=CodexNotia'));
    assert.ok(requests[0].includes('%E5%86%85%E9%83%A8%E9%94%99%E8%AF%AF%E6%B6%88%E6%81%AF'));

    const logContent = await fs.readFile(service.logFilePath, 'utf8');
    assert.equal(logContent.includes('内部错误通知发送失败，已停止继续重试'), true);
    assert.equal(
      logContent.match(/内部错误通知发送失败，已停止继续重试/g)?.length ?? 0,
      1
    );
  } finally {
    await new Promise((resolve, reject) => {
...
/**
 * 验证同类项目内部错误在安静一段时间后会被视作新的一轮。
 * 这样既不会无限重试，也不会把后续真正新的故障永久压掉。
 */
test('项目内部异常在冷却静默后会开启新一轮尝试', async () => {
  const sessionsDir = await createTempDir();
  const runtimeDir = await createTempDir();
  let requestCount = 0;
  const server = http.createServer((request, response) => {
  ...
  try {
    ...
    config.push.maxAttempts = 1;
    config.service.internalErrorNotifyCooldownMs = 20;
    const service = new CodexNotiaService(config);
    await service.stateStore.initialize();

    await service.reportInternalError('测试内部异常', new Error('内部错误消息'));

    const fingerprint = '测试内部异常:内部错误消息';
    service.internalErrorHistory.get(fingerprint).lastObservedAt = Date.now() - 21;
...
/**
 * 验证首次启动按历史基线接管旧会话时，
 * 已经过了超时点的旧未完成 turn 不会在启动瞬间被误发成新的超时错误。
 */
test('首次启动跳过历史会话时，旧 pending turn 不会立刻触发超时通知', async () => {
  const sessionsDir = await createTempDir();
  const runtimeDir = await createTempDir();
  const capture = await startCaptureServer();
  ...
  try {
    const config = buildConfig({ sessionsDir, runtimeDir, port: capture.port });
    config.service.ignoreHistoricalSessionsOnFirstRun = true;
    config.service.staleTurnTimeoutMs = 1000;
    const service = new CodexNotiaService(config);
    await service.stateStore.initialize();

    const historicalStartedAt = new Date(Date.now() - (10 * 60 * 1000)).toISOString();
    const filePath = await writeSessionFile(sessionsDir, 'startup-pending-history.jsonl', [
      JSON.stringify({
        timestamp: historicalStartedAt,
        type: 'session_meta',
        payload: {
    ...
    assert.equal(capture.requests.length, 0);
    const fileState = service.stateStore.getState().files[filePath];
    assert.ok(fileState.turns['turn-startup-pending']);
    assert.ok(fileState.turns['turn-startup-pending'].timeoutSuppressedAt);
  } finally {
    await capture.close();
  }
});
...
/**
 * 验证从状态文件恢复出的旧未完成 turn，
 * 在重启后的首轮扫描里也不会被立即误判成新的超时错误。
 */
test('恢复旧状态后的 pending turn 不会在启动首轮扫描里立刻超时', async () => {
  const sessionsDir = await createTempDir();
  const runtimeDir = await createTempDir();
  const capture = await startCaptureServer();
  ...
  try {
    ...
    config.service.staleTurnTimeoutMs = 1000;
    const historicalStartedAt = new Date(Date.now() - (10 * 60 * 1000)).toISOString();
    const filePath = await writeSessionFile(sessionsDir, 'restored-pending.jsonl', [
      JSON.stringify({
        timestamp: historicalStartedAt,
        type: 'session_meta',
        payload: {
          id: 'session-restored-pending',
          originator: 'Codex Desktop',
          source: 'vscode'
        }
      }),
      JSON.stringify({
        ...
        payload: {
          type: 'task_started',
          turn_id: 'turn-restored-pending'
        }
      })
    ]);

    const initialService = new CodexNotiaService(config);
    await initialService.stateStore.initialize();
    initialService.stateStore.getState().bootstrapComplete = true;
    initialService.stateStore.getState().files[filePath] = {
      filePath,
      offset: (await fs.stat(filePath)).size,
      remainder: '',
      sessionId: 'session-restored-pending',
      originator: 'Codex Desktop',
      source: 'vscode',
      currentTurnId: 'turn-restored-pending',
      lastEventAt: historicalStartedAt,
      latestFinalAnswerText: '',
      latestErrorMessage: '',
      turns: {
        'turn-restored-pending': {
    ...
    await initialService.stateStore.save();

    const restoredService = new CodexNotiaService(config);
    await restoredService.stateStore.initialize();
    await restoredService.scanOnce();

    assert.equal(capture.requests.length, 0);
    const restoredFileState = restoredService.stateStore.getState().files[filePath];
    assert.ok(restoredFileState.turns['turn-restored-pending'].timeoutSuppressedAt);
  } finally {
    await capture.close();
  }
});
...
/**
 * 验证被启动隔离保护过的旧活动 turn，
 * 只要在当前进程启动后重新活动，就会重新进入实时超时跟踪。
 */
test('启动隔离的旧 pending turn 在重新活动后仍会正常触发超时通知', async () => {
  const sessionsDir = await createTempDir();
  const runtimeDir = await createTempDir();
  const capture = await startCaptureServer();
  ...
  try {
    ...
    const historicalStartedAt = new Date(Date.now() - (10 * 60 * 1000)).toISOString();
    const filePath = await writeSessionFile(sessionsDir, 'startup-pending-reactivated.jsonl', [
      JSON.stringify({
        timestamp: historicalStartedAt,
        type: 'session_meta',
        payload: {
    ...
    assert.equal(capture.requests.length, 0);
    assert.ok(service.stateStore.getState().files[filePath].turns['turn-reactivated'].timeoutSuppressedAt);

    await new Promise((resolve) => {
      setTimeout(resolve, 10);
    });

    const liveEventAt = new Date().toISOString();
    await fs.appendFile(filePath, [
      JSON.stringify({
        timestamp: liveEventAt,
        type: 'event_msg',
        payload: {
          type: 'agent_message',
          turn_id: 'turn-reactivated',
    ...
    service.config.service.staleTurnTimeoutMs = 10 * 24 * 60 * 60 * 1000;
    await service.scanOnce();

    const turnState = service.stateStore.getState().files[filePath].turns['turn-reactivated'];
    assert.equal(turnState.timeoutSuppressedAt, '');
